...
getHeadBlockNum()
{
  INFO="$(programs/teclos/teclos get info)"
  verifyErrorCode "teclos get info"
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/teclos/teclos --port $port2 get block 1)"
    verifyErrorCode "teclos get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/teclos/teclos create key)"
verifyErrorCode "teclos create key"
...
PASSWORD_INITA="$(programs/teclos/teclos wallet create --name inita)"
verifyErrorCode "teclos wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
programs/teclos/teclos wallet import --name inita --private-key $INITA_PRV_KEY
verifyErrorCode "teclos wallet import"
programs/teclos/teclos wallet import --name inita --private-key $PRV_KEY1
verifyErrorCode "teclos wallet import"
programs/teclos/teclos wallet import --name inita --private-key $PRV_KEY2
verifyErrorCode "teclos wallet import"
programs/teclos/teclos wallet import --name inita --private-key $PRV_KEY3
verifyErrorCode "teclos wallet import"
...
ACCOUNT_INFO="$(programs/teclos/teclos create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "teclos create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/teclos/teclos get account testera)"
verifyErrorCode "teclos get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/teclos/teclos transfer inita testera 975321 "test transfer")"
    verifyErrorCode "teclos transfer"
   ...
   TRANS_INFO="$(programs/teclos/teclos --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "teclos get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done
