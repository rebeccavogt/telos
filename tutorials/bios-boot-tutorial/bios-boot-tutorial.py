...
def startWallet():
    ...
    sleep(.4)
    run(args.teclos + 'wallet create')

def importKeys():
    run(args.teclos + 'wallet import --private-key ' + args.private_key)
    keys = {}
    for a in accounts:
        key = a['pvt']
        if not key in keys:
            if len(keys) >= args.max_user_keys:
                break
            keys[key] = True
            run(args.teclos + 'wallet import --private-key ' + key)
...
def createSystemAccounts():
    for a in systemAccounts:
        run(args.teclos + 'create account eosio ' + a + ' ' + args.public_key)
...
def createStakedAccounts(b, e):
    ...
    for i in range(b, e):
        ...
        retry(args.teclos + 'system newaccount --transfer eosio %s %s --stake-net "%s" --stake-cpu "%s" --buy-ram "%s"   ' %
            (a['name'], a['pub'], intToCurrency(stakeNet), intToCurrency(stakeCpu), intToCurrency(ramFunds)))
        if unstaked:
            retry(args.teclos + 'transfer eosio %s "%s"' % (a['name'], intToCurrency(unstaked)))

def regProducers(b, e):
    for i in range(b, e):
        a = accounts[i]
        retry(args.teclos + 'system regproducer ' + a['name'] + ' ' + a['pub'] + ' https://' + a['name'] + '.com' + '/' + a['pub'])

def listProducers():
    run(args.teclos + 'system listproducers')
...
def vote(b, e):
    for i in range(b, e):
        ...
        retry(args.teclos + 'system voteproducer prods ' + voter + ' ' + prods)

def claimRewards():
    table = getJsonOutput(args.teclos + 'get table eosio eosio producers -l 100')
    times = []
    for row in table['rows']:
        if row['unpaid_blocks'] and not row['last_claim_time']:
            times.append(getJsonOutput(args.teclos + 'system claimrewards -j ' + row['owner'])['processed']['elapsed'])
    print('Elapsed time for claimrewards:', times)

def proxyVotes(b, e):
    vote(firstProducer, firstProducer + 1)
    proxy = accounts[firstProducer]['name']
    retry(args.teclos + 'system regproxy ' + proxy)
    sleep(1.0)
    for i in range(b, e):
        voter = accounts[i]['name']
        retry(args.teclos + 'system voteproducer proxy ' + voter + ' ' + proxy)

def updateAuth(account, permission, parent, controller):
    run(args.teclos + 'push action eosio updateauth' + jsonArg({
        'account': account,
...
def resign(account, controller):
    ...
    sleep(1)
    run(args.teclos + 'get account ' + account)
...
def randomTransfer(b, e):
    for j in range(20):
        ...
        run(args.teclos + 'transfer -f ' + src + ' ' + dest + ' "0.0001 ' + args.symbol + '"' + ' || true')
...
def msigProposeReplaceSystem(proposer, proposalName):
    ...
    run(args.teclos + 'multisig propose ' + proposalName + jsonArg(requestedPermissions) +
        jsonArg(trxPermissions) + 'eosio setcode' + jsonArg(setcode) + ' -p ' + proposer)

def msigApproveReplaceSystem(proposer, proposalName):
    for i in range(firstProducer, firstProducer + numProducers):
        run(args.teclos + 'multisig approve ' + proposer + ' ' + proposalName +
            jsonArg({'actor': accounts[i]['name'], 'permission': 'active'}) +
            '-p ' + accounts[i]['name'])

def msigExecReplaceSystem(proposer, proposalName):
    retry(args.teclos + 'multisig exec ' + proposer + ' ' + proposalName + ' -p ' + proposer)

def msigReplaceSystem():
    run(args.teclos + 'push action eosio buyrambytes' + jsonArg(['eosio', accounts[0]['name'], 200000]) + '-p eosio')
    sleep(1)
...
def produceNewAccounts():
    with open('newusers', 'w') as f:
        for i in range(120_000, 200_000):
            x = getOutput(args.teclos + 'create key --to-console')
...
def stepInstallSystemContracts():
    run(args.teclos + 'set contract eosio.token ' + args.contracts_dir + 'eosio.token/')
    run(args.teclos + 'set contract eosio.msig ' + args.contracts_dir + 'eosio.msig/')
def stepCreateTokens():
    run(args.teclos + 'push action eosio.token create \'["eosio", "10000000000.0000 %s"]\' -p eosio.token' % (args.symbol))
    totalAllocation = allocateFunds(0, len(accounts))
    run(args.teclos + 'push action eosio.token issue \'["eosio", "%s", "memo"]\' -p eosio' % intToCurrency(totalAllocation))
    sleep(1)
def stepSetSystemContract():
    retry(args.teclos + 'set contract eosio ' + args.contracts_dir + 'eosio.system/')
    sleep(1)
    run(args.teclos + 'push action eosio setpriv' + jsonArg(['eosio.msig', 1]) + '-p eosio@active')
...
parser.add_argument('--teclos', metavar='', help="Cleos command", default='../../build/programs/teclos/teclos --wallet-url http://localhost:6666 ')
...
parser.add_argument('-H', '--http-port', type=int, default=8000, metavar='', help='HTTP port for teclos')
...
args.teclos += '--url http://localhost:%d ' % args.http_port
